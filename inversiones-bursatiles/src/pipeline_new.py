...
def run_pipeline():
    ...
    try:
        ...
        index_stats = {
            "merval":  get_index_stats(merval_df,  _idx_col(merval_df,  "MERVAL")  or _idx_col(merval_df,  "INDICE") or ""),
            "bovespa": get_index_stats(bovespa_df, _idx_col(bovespa_df, "BOVESPA") or _idx_col(bovespa_df, "INDICE") or ""),
            "sp500":   get_index_stats(sp500_df,   _idx_col(sp500_df,   "S&P")     or _idx_col(sp500_df,   "INDICE") or ""),
        }

        empty_markets = [k for k, v in index_stats.items() if not v or v.get("actual", 0) == 0]
        if len(empty_markets) == 3:
            raise RuntimeError(
                f"index_stats vacio para los 3 mercados. "
                f"Columnas merval={list(merval_df.columns[:5])}"
            )
        if empty_markets:
            logger.warning(f"index_stats vacio para: {empty_markets}")

        # 4. DETECCION DE CAMBIOS
        ...
        pub_ok = publish_dashboard(dashboard_path, dashboard_name)
        if pub_ok:
            logger.info("Dashboard publicado en GitHub Pages correctamente")
        else:
            logger.warning("No se pudo publicar en GitHub Pages")

        # 6. EXCEL
        excel_path = None
        if SEND_EXCEL:
            logger.info("6/7 Generando Excel...")
