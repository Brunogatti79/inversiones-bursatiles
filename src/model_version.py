...
CHANGELOG = [
    ...
    },
    {
        ...
        "date": "2026-07-27",
        "changes": [
            "Fix: atr_metodo, vol_regime_mercado y v2_regime_mult (agregados a "
            "analyzer.py en v4.10/v4.11) no se persistían en "
            "signals_history.json -- tracker.update_history() tiene un "
            "allowlist explícito de campos (para no inflar el historial) y no "
            "se había actualizado al agregar esos 2 fixes. La señal en vivo "
            "SIEMPRE los calculó bien; solo faltaba copiarlos al historial.",
            "Encontrado auditando la primera corrida real end-to-end post-"
            "deploy de v4.10/v4.11 (commit d06b2f0, 27/07/2026 17:38 UTC): los "
            "67 registros de ese día tenían atr_metodo=None y "
            "vol_regime_mercado=None pese a que download_status.json ya "
            "confirmaba ohlc=true para los 3 mercados -- el dato real estaba, "
            "el historial no lo guardaba.",
            "Mismo commit real que confirmó, de paso, que el fix de CEDEAR "
            "(v4.11) funcionó en producción: download_status.json pasó de "
            "CEDEAR ok=false a ok=true, success=true en los 4 mercados.",
            "Tests: +1 archivo nuevo (test_tracker_ohlc_regime_fields.py, 5 "
            "tests) fijando estos 3 campos en el historial y su default "
            "seguro ante señales incompletas. Suite completa: 599/599 verde.",
        ],
    },
    {
        "version": "4.11",
    ...
    },
    {
        ...
        "date": "2026-06-26",
        "changes": [
            "Feat CRITICO: pricing real de CEDEAR via data912.com (API "
            "publica, sin autenticacion, sin costo -- confirmada en uso por "
            "herramientas argentinas reales como rendimientos.co/cedears). "
            "En la v4.7 (mismo dia, sesion anterior) se intento la "
            "aproximacion nyse_usd x ratio_cedear y se descarto por dar "
            "numeros absurdos; la causa de fondo era que no existe ninguna "
            "fuente de precio CEDEAR real en el repo. data912 resuelve esto "
            "de raiz: para cada ticker, su endpoint /live/arg_cedears trae "
            "tanto la linea ARS del CEDEAR como, para la mayoria, una linea "
            "'{ticker}D' -- un instrumento DISTINTO, liquidado directo en "
            "dolares en BYMA (dolar-MEP). Usar esa linea D da el precio USD "
            "real de mercado sin necesitar ratio_cedear ni CCL para nada -- "
            "elimina la ambiguedad de ratio que rompio el intento anterior. "
            "Confirmado en vivo: GLOB c=2376 ARS / GLOBD c=1.61 USD, mismo "
            "patron en IBB, EWZ, COPX y la mayoria del resto. Cuando un "
            "ticker no tiene linea D (CEDEAR poco operado), cae a ARS/CCL "
            "como respaldo -- sigue siendo precio real de mercado.",
            "Feat: las 8 posiciones CEDEAR reales (GLOB, MELI, MSFT, COPX, "
            "IBB, PBR, RIO, EWZ) ahora SI reciben stop_loss/target al "
            "comprar (risk_engine.compute_initial_stop_target) -- usando el "
            "ATR de la senal NYSE aplicado directo en USD. Aproximacion "
            "razonable (mismo activo subyacente, volatilidad en dolares "
            "correlacionada) pero no es el ATR de la serie real del CEDEAR "
            "en BYMA -- queda marcado con metodo='atr_nyse_aplicado_a_"
            "cedear_*' para que sea auditable si en el futuro se quiere "
            "calcular sobre la serie historica real del CEDEAR.",
            "Feat: data/cedear_cierres.csv -- snapshot diario del precio "
            "CEDEAR persistido a GitHub (mismo patron que el resto del "
            "proyecto: filesystem de Railway efimero). Sirve de respaldo "
            "si data912 esta caido en el momento de un run; agregado a "
            "sync_all_at_startup.",
            "Tests: +13 (resolve_position_price con cedear real: 2, "
            "fetch_live_cedear_usd_prices -- primera suite, mockeando la "
            "respuesta HTTP sin red real: 4, stop/target CEDEAR habilitado: "
            "1, contrato de sync: 1; ajustados 3 tests existentes a la "
            "nueva firma de cedear_prices keyed-por-ticker-en-USD en vez de "
            "keyed-por-nombre-en-ARS). Suite completa: 449/449 verde.",
            "Pendiente: el ATR aplicado a CEDEAR usa la serie NYSE, no la "
            "serie real del CEDEAR en BYMA -- data912 no expone OHLC "
            "historico facil de descargar todavia (solo live), así que "
            "calcular un ATR propio del CEDEAR queda para una sesion "
            "futura si la aproximacion actual resulta insuficiente en la "
            "practica.",
        ],
    },
    {
