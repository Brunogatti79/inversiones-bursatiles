...
def _signal_section(signals: list, market: str) -> str:
...
def _index_line(stats: dict, market: str) -> str:
    ...
    return (f"{flag} <b>{market}</b> {actual:,.0f}  "
...
def send_daily_report(all_signals, index_stats, dashboard_filename, run_date=None):
    ...
    # Link a GitHub Pages
    dash_url = f"https://{GH_USER}.github.io/{GH_REPO}/outputs/{dashboard_filename}"
...
def send_startup_message():
    ...
    text = (
        ...
        f"  /senales — Ver señales actuales\n"
        f"  /help — Ayuda"
    )
    return _send_message(text)
