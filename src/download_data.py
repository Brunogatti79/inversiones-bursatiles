...
# FIX 03/09/2026 (root cause real de la corrupción de sp500_cierres.csv,
# no confundir con el fix del mismo día en src/downloader.py -- son dos
# bugs DISTINTOS en dos scripts distintos). Confirmado con git log +
# git show en el repo real: el commit 42ad8386 (github-actions[bot],
# 2026-09-03 00:59 UTC) pisó la fila 2026-09-02 -- que en el commit
# anterior (a473315f, 22:23 UTC) tenía datos completos y buenos -- con una
# fila casi vacía (1/40 columnas con dato, solo el índice). Mecanismo real:
# MIN_SUCCESS_RATE ya validaba el % de TICKERS con una serie históricamente
# usable (>=MIN_ROWS días en total), pero un ticker puede pasar ese check
# perfectamente (300+ días de historia buena) y aun así no tener el día
# MÁS RECIENTE todavía publicado por Yahoo en el momento exacto de la
# request (dato que sale más tarde para acciones individuales que para el
# índice) -- pd.concat() con outer join arma la fila de todas formas,
# con NaN en cada ticker que no llegó a tiempo. Nada validaba la cobertura
# de esa ÚLTIMA fila específicamente antes de guardar/commitear.


def _drop_incomplete_last_row(df: pd.DataFrame, market_name: str,
                               threshold: float = MIN_SUCCESS_RATE) -> pd.DataFrame:
    """
    Si la última fila del DataFrame tiene menos de `threshold` de sus
    columnas con dato, la descarta antes de guardar/commitear -- en vez de
    persistir una fecha casi vacía que después data_validator.py va a
    marcar como ERROR de todas formas, pero ya tarde (con el CSV malo ya
    en git). No es lo mismo que fallar todo el mercado: el resto de los
    ~275-400 días de historia sigue siendo bueno, se pierde nada más el día
    más reciente si vino incompleto -- se recupera solo en la próxima
    corrida (cron cada ~2-3h) cuando Yahoo ya haya publicado ese día para
    el resto de los tickers.
    """
    if df is None or df.empty:
        return df
    ultima_fila = df.iloc[-1]
    total_cols = len(df.columns)
    con_dato = int(ultima_fila.notna().sum())
    tasa = con_dato / total_cols if total_cols else 0
    if tasa < threshold:
        fecha = df.index[-1]
        logger.warning(
            f"[{market_name}] Última fila ({fecha.date()}) con cobertura insuficiente "
            f"({con_dato}/{total_cols}, {tasa:.0%}) -- se descarta, se recupera en la próxima corrida"
        )
        return df.iloc[:-1]
    return df
...
def download_market(tickers, index_ticker, market_name):
    ...
    df = _drop_incomplete_last_row(df, market_name)
    ...
    if series_high_list:
        ...
        # Mismo criterio de última fila que Close, y ADEMÁS se recorta al
        # índice final de df (que puede haber perdido su última fecha
        # arriba) para que Close/High/Low no queden desalineados por fecha.
        df_high = _drop_incomplete_last_row(df_high, f"{market_name} (high)")
        df_high = df_high.reindex(df.index)
    ...
    if series_low_list:
        ...
        df_low = _drop_incomplete_last_row(df_low, f"{market_name} (low)")
        df_low = df_low.reindex(df.index)
